   ...
   withSelectedImg:(UIImage *)selectedImg
{
    if (self = [super initWithFrame:frame]) {
        ...
        //存放标题和icon图标的view
        self.itemView =  [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width/5, 49)];
        [self addSubview:self.itemView];
...
//
        //icon 图标
        self.tabbarImgView = [[UIImageView alloc]initWithImage:unSelectedImg];
        self.tabbarImgView.frame = CGRectMake(0, 0, 60, 50) ;
        self.tabbarImgView.contentMode =  UIViewContentModeCenter;
//        self.tabbarImgView.center  =CGPointMake(self.titleLabel.center.x, self.titleLabel.center.y-22);
//        self.tabbarImgView.center=CGPointMake(self.frame.origin.x+self.frame.size.width/2, self.frame.origin.y+self.frame.size.height/2);
        self.tabbarImgView.tag = 1001 ;
        [self.itemView addSubview:self.tabbarImgView];
